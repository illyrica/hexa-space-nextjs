...
export const ShipComponent = ({ ship, persistClapInc }: { ship: SpaceShip; persistClapInc: incClaps }) => {
  ...
  return (
    <>
      <div className="ship">
        <div>
          <Image src={`/images${ship.image}`} height={256} width={256} alt={ship.name} priority />
          <p>{ship.name}</p>
        </div>
        <ul className="ship-data">
          ...
          {ship.mileage && (
            <li data-testid="ship-mileage">
              <b>Mileage</b>: <span> {ship.mileage} ly</span>
            </li>
          )}
          {ship.speed && (
            <li data-testid="ship-speed">
          ...
          <li>
            <b>Monthly Rate</b>:<span data-testid="monthly-rate"> {monthlyRate(ship, numberOfRates).toFixed(2)}</span>
          </li>
